...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const dateBetween = new Date(endDate.getTime() - startDate.getTime());

  return dateBetween.getTime() / (1000 * 60 * 60 * 24) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return (
    ...
    newDate.getTime() <= endDate.getTime()
  );
}
...
function formatDate(date) {
  ...
  const fullYear = newDate.getUTCFullYear();
  ...
  const middayHour = hour % 12 || 12;

  return `${month}/${day}/${fullYear}, ${middayHour}:${String(minute).padStart(2, '0')}:${String(second).padStart(2, '0')} ${midday}`;
}
...
function getWeekNumberByDate(date) {
  ...
  let weekCounter = newDate.getDay() === 1 ? 0 : 1;
  ...
  while (newDate.getTime() <= date.getTime()) {
    if (newDate.getDay() === 1) weekCounter += 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  let currentWorkDaysCounter = 0;
  let currentOffDaysCounter = 0;
  const shedule = [];

  while (startDate.getTime() <= endDate.getTime()) {
    if (currentWorkDaysCounter < countWorkDays) {
      const day = String(startDate.getDate()).padStart(2, '0');
      const month = String(startDate.getMonth() + 1).padStart(2, '0');
      const year = startDate.getFullYear();

      shedule.push(`${day}-${month}-${year}`);
      currentWorkDaysCounter += 1;

      startDate.setDate(startDate.getDate() + 1);
    } else if (currentOffDaysCounter < countOffDays) {
      currentOffDaysCounter += 1;

      startDate.setDate(startDate.getDate() + 1);
    } else {
      currentOffDaysCounter = 0;
      currentWorkDaysCounter = 0;
    }
  }
...
function isLeapYear(date) {
  ...
  const daysInFeb = newDate.getDate();
  return daysInFeb === 29;
}
